...
public class InitMongoDBDataChangeLog {
    ...
    public void initBooks(BookRepository repository, AuthorRepository authorRepository){
        repository.saveAll(List.of(
                new Book("test book 1", 1995,
                        List.of(
                                new Genre("test genre 1")),
                        authorRepository.findByName("test author")),
                new Book("test book 2", null,
    ...
    public void initComments(CommentRepository repository, BookRepository bookRepository){
        repository.saveAll(List.of(
                new Comment("test content 1", "test user 1", bookRepository.findByName("test book 1")),
                new Comment("test content 2", "test user 2", bookRepository.findByName("test book 1")),
                new Comment("test content 3", "test user 3", bookRepository.findByName("test book 2"))
        ));
    }
}
